---
draft: false
from: mat
title: Surfers Against Sewage
heading: Let’s protect clean water, together
subheading: We’re Kind, an Ocean Network Member, and we’d love to support the amazing work you do to protect the ocean and tackle sewage pollution. Your mission aligns perfectly with the kind of brands we support and we’d love to work together, using our expertise to help grow awareness and get more people supporting Surfers Against Sewage.
cover_img: images/work-rivers-trust-phone-2.jpg
cover_img_alt: Shot in the environment, a mobile-optimised website for The Rivers Trust website
banner_images:
- img: images/work-twp-hero-3.jpg
  alt: A person stands beside two canoes on the shore of a misty river at sunrise, surrounded by reeds and trees.
- img: images/work-twp-outdoor-laptop.jpg
  alt: The Wilderness Project website shown on a laptop. The laptop is situated outdoors, on a rock
- img: images/work-rewilding-britain-carousel-2.jpg
  alt: A beaver on a river bank
- img: images/work-rewilding-britain-carousel-4.jpg
  alt: Landscape image of a large tree on farmland
logo_wrapperclass: w-12 lg:w-24 xl:w-36
logo:
- name: Surfers Against Sewage
  icon: logo-sas
  icon_width: 355
  icon_height: 139
related_projects:
  - slug: work/the-rivers-trust
  - slug: work/rewilding-britain
  - slug: work/the-wilderness-project
  - slug: work/plan-vivo
related_posts:
  - strategy-for-social-change
  - creating-sustainable-and-ethical-digital-products
  - brand-strategy-make-an-impact
testimonials:
- testimonials/rwb-short
- testimonials/plan-vivo
related_stats:
- pay-gap
- socially-reponsible
- charity-donations
- gender-neutral
extra_stats:
- 23k-donated
- 28-percent
help_intro: When you’ve got a lot going on, your messaging, campaigns and digital can lose consistency over time. These are four ways we can help bring it back into focus, without creating extra complexity for your team.
help_items:
- title: Campaign focus
  description: We can help you define a clear campaign hierarchy, so every campaign reinforces the same priorities and makes your work feel cumulative, not fragmented.
- title: Supporter journeys
  description: Whether someone wants to join, donate, volunteer, take part in citizen science or share your work, the path should feel simple and rewarding. We can identify where people drop off and make improvements that increase participation and long-term support.
- title: Content structure
  description: We can help organise your content so it’s easier for people to find what they need, understand the issue quickly, and take the right next step.
- title: Campaign system
  description: Instead of reinventing the wheel each time, we can help you create a repeatable structure for campaigns, landing pages and toolkits. That means faster launches, clearer calls to action, and a more consistent experience for everyone engaging with your work.
help_outro: None of this needs to start with a big project. The goal is to help you make a few smart decisions that unlock better results across everything you already do.
---
