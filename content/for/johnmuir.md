---
draft: false
title: John Muir Trust
heading: We’d like to help keep wild places wild
subheading: We’re Kind, and we’d love to support the amazing work you do to protect wild places and help more people connect with nature. Your mission aligns perfectly with the kind of brands we support and we’d love to work together, using our expertise to help grow awareness and get more people supporting the John Muir Trust.
cover_img: images/work-rivers-trust-phone-2.jpg
cover_img_alt: Shot in the environment, a mobile-optimised website for The Rivers Trust website
banner_images:
- img: images/work-twp-hero-3.jpg
  alt: Multiple canoes on a calm river that reflects the colorful sunset sky.
- img: images/work-twp-outdoor-laptop.jpg
  alt: The Wilderness Project website shown on a laptop. The laptop is situated outdoors, on a rock
- img: images/Quinag_002.jpg
  alt: A view from Quinag
- img: images/work-rewilding-britain-teaser.jpg
  alt: A Blue Tit on a branch
logo_wrapperclass: w-12 lg:w-24 xl:w-36
logo:
- name: John Muir Trust
  icon: logo-johnmuir
  icon_width: 64
  icon_height: 64
related_projects:
  - slug: work/the-wilderness-project
  - slug: work/rewilding-britain
  - slug: work/the-rivers-trust
  - slug: work/plan-vivo
related_posts:
  - brand-strategy-make-an-impact
  - strategy-for-social-change
  - communicating-complex-issues
testimonials:
- testimonials/twp-short
- testimonials/rwb-short
related_stats:
- pay-gap
- socially-reponsible
- charity-donations
- gender-neutral
extra_stats:
- 23k-donated
- 28-percent
help_intro: We don’t know all of your challenges yet, but we work with charities like yours every day. Here are a few ways we can help bring clarity and consistency, without creating extra complexity for your team.
help_items:
- title: Membership clarity
  description: We can help strengthen the membership proposition, so it’s clear why joining matters, what members enable, and what they get in return.
- title: Impact storytelling
  description: You’ve already got a strong mission and real impact behind it. We can help shape that into a story that’s easy to understand, easy to share, and helps people feel confident supporting you.
- title: Supporter journeys
  description: Whether someone wants to join, donate, volunteer or share your work, the path should feel simple and rewarding. We can help spot where people drop off, then make improvements that increase participation and long-term support.
- title: Content structure
  description: We can help organise your content so it’s easier for people to find what they need, understand your work quickly, and take the right next step.
help_outro: If any of this sounds useful, we’d love to share a bit more about how we work and see if it feels like a good fit.
---
